...
class Mongo extends Driver
{
    ...
    public function select($options = array())
    {
        ...
        try {
            ...
            if(isset($options['limitGroup'])){
                ...
                $field = $options['limitGroup'];
                if ($this->config['debug']) {
                    $this->queryStr .= '.limitGroup('.$field.',' . intval($length) . ')';
                }
                ...
                while($_cursor->hasNext()){
                	...
                	$tmp = $_cursor->next();
                	if($field && isset($tmp[$field]) && $tmp[$field]){
                		if(!isset($resultSet[$tmp[$field]])){
                			$resultSet[$tmp[$field]] = $tmp;
                		}
                	}
                	else{
                		$resultSet[strval($tmp['_id'])] = $tmp;
                	}
                }
    ...
    public function aggregate($pipeline, $options=array()){
        ...
        try{
            ...
            if ($cache && $aggregate['ok']) {
                S($key, $aggregate, $cache['expire'], $cache['type']);
            }
