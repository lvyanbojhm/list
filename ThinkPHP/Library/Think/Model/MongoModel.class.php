...
class MongoModel extends Model
{
    ...
    public function save($data = '', $options = array()){
    	...
    	if(is_array($result)){
    		if(isset($result['upserted'])){//添加
    			if(!$result['upserted']) return false;
    			$data[$this->getPk()] = $result['upserted'];
    			$this->_after_insert($data, $options);
    			return $result['upserted'];
    		}
    		else{
    			if(!isset($result['nModified'])) return false;
    ...
    public function group($key, $init, $reduce, $option = array())
    {
        ...
        if(!isset($option['finalize'])){
        	$option['finalize'] = 'function(k,v){}';
        }
    ...
    //分组 分页
    public function fetchGroup($group, $where = array(), $page = 1, $page_size = 20, $sort = false, $field = false){
    	if(!$group){
    		return  false;
    	}
    	...
    	if($field){
    		...
    		if(array_diff($group, $field)){
    			throw new \Exception('group must be in field');//group中有字段 field 中没有
    		}
    		
    		$pipe[] = array('$project'=>array_combine($field, array_fill(0, count($field), 1)));
    	}
    	...
    	if($where){
    		...
    		if($field && array_diff(array_keys($where), $field)){
    			throw new \Exception('condition must be in field');
    		}
    	...
    	$opt['group_count'] = array('$sum'=>1);
    	$opt['item'] = array('$first'=>'$$ROOT');
    	$pipe[] = array('$group'=>$opt);
    	
    	$skip = ($page - 1) * $page_size;
    	if($skip > 0){
    		$pipe[] = array('$skip'=>$skip);
    	}
    	$pipe[] = array('$limit'=>$page_size);
    	
    	//var_dump($pipe);exit;
        $ret = $this->aggregate($pipe);
        if(!isset($ret['ok']) || !$ret['ok'] || !isset($ret['result']) || !$ret['result']){
        	return false;
        }
        ...
        foreach ($ret['result'] as $row){
        	$tmp = $row['item'];
        	$tmp['group_count'] = $row['group_count'];
        	$ret_new[] = $tmp;
        }
        
        $this->_after_select($ret_new, array());
        return $ret_new;
    }
