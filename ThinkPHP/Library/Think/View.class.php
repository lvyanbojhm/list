...
class View
{
    ...
    public function parseTemplate($template = '')
    {
        ...
        if($module){
        	$tpl_real_path = $this->getThemePath($module);
        }
        else{
        	// 获取当前主题的模版路径
        	$tpl_real_path = THEME_PATH;
        }
        ...
        $file = $tpl_real_path . $template . C('TMPL_TEMPLATE_SUFFIX');
        if (C('TMPL_LOAD_DEFAULTTHEME') && THEME_NAME != C('DEFAULT_THEME') && !is_file($file)) {
            // 找不到当前主题模板的时候定位默认主题中的模板
            $file = dirname($tpl_real_path) . '/' . C('DEFAULT_THEME') . '/' . $template . C('TMPL_TEMPLATE_SUFFIX');
        }
        return $file;
    }
