 ...
 * @return string
 */
function U($url = '', $vars = '', $suffix = true, $domain = false, $url_common = false)
{
    // 解析URL
    $info = parse_url($url);
	...
	if((!isset($vars['wxid']) || !$vars['wxid']) && (!isset($var['wxid']) || !$var['wxid'])){
		$vars['wxid'] = C('wx_account_id');
    }
    
    if ((0 == C('URL_MODEL')) || $url_common) {
        // 普通模式URL转换
        ...
        if (!empty($vars)) {
            ...
            foreach ($vars as $var => $val) {
            	if(is_array($val)){
            		foreach($val as $k=>$v){
	            		if ('' !== trim($v)) {
		                    $url .= $depr . $var."[{$k}]" . $depr . urlencode($v);
		                }
            		}
            	}
            	else{
